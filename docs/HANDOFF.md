...
3. **Featured projects** — `#home-featured` ✅ built (our `#featured`). Real: 9 project
   ...
   flood pill. Ours uses six real product case studies.
   **Live re-study 2026-06-11 (fidelity pass applied):** the type pill
   ...
   like a small camera-POV move. The product-media upgrade keeps that treatment and
   fades the real captured `thumbVideo` over the still on hover/keyboard focus; touch
   and reduced-motion users retain the still. Media stays over-scaled 1.12 and quickTo
   x/y pans ±3% of tile size opposite the cursor. The hovered item slides the arrow+name 1em right (real
